...
attains $-\infty$ and attains finite value at least one point. If $f$ is a
...
\|x\|_1 = 1 \}$ and receives a loss\footnote{The reason for using loss, instead
of reward, will be apparent in Section~\ref{section:reduction-experts}.} vector
$\ell_t \in [0,1]^N$. Similarly as before, the goal of the algorithm is to
minimize its regret after $T$ rounds with respect to any competitor $u \in
\Delta_N$, which is defined as
$$
...
replace $p$ with an estimate. This problem was studied in information theory by
\citet{KrichevskyT81}, who proposed that after seeing coin flips $g_1, g_2,
\dots, g_{t-1}$ insteaf of $p$ we use an empirical estimate\footnote{Compared
the more common maximum likelihood estimate $\frac{\sum_{i=1}^{t-1} \indicator[g_i = +1]}{t-1}$,
the KT estimator ``shrinks'' slightly towards $\frac{1}{2}$.}
$k_t = \frac{1/2 + \sum_{i=1}^{t-1} \indicator[g_i = +1]}{t}$.
This results in betting strategy
\begin{equation}
...
$$
\forall \beta \in [-1,1] \qquad \qquad \ln(\Wealth_t) \ge \ln(\Wealth_t(\beta)) \ - \ \frac{1}{2} \ln t \ - \ ??? \; ,
$$
