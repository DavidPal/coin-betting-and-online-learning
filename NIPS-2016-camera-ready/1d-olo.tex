...
\eqref{equation:wealth-recurrence} holds. We will see that the restriction in
\eqref{equation:one-dimensional-olo} does not prevent us from obtaining
parameter-free algorithms with optimal bounds.  If we have a coin-betting
algorithm that, on a sequence of coin flips $\{g_t\}_{t=1}^\infty$, $g_t \in
[-1,1]$, bets fractions $\beta_t \in [-1,1]$, we can use it to construct an OLO
algorithm in a one-dimensional Hilbert space $\R$ according to
