...
\documentclass{colt2016} % Anonymized submission
...
\begin{abstract}
In the recent years a number of parameter-free algorithms for online linear optimization over
...
a simple algorithm for online linear optimization with $\widetilde O(\|u\|\sqrt{T
\log\|u\|})$ regret with respect to any competitor $u$. For learning with
expert advice for any prior $\pi$ we obtain an algorithm that has $O(\sqrt{T
(1 + \KL{u}{\pi})})$ regret against any competitor $u$. In both cases, no parameters need to be tuned.
\end{abstract}
