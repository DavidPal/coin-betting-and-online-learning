...
a simple algorithm for online linear optimization with $O(\|u\|\sqrt{T
\log(1+T\|u\|}))$ regret with respect to any competitor $u$. For learning with
expert advice we obtain an algorithm that has $O(\sqrt{T (1 + \KL{u}{\pi})})$
regret against any competitor $u$ and where $\pi$ is a prior distribution given
to the algorithm as an input. In both cases, no parameters need to be tuned.
\end{abstract}
