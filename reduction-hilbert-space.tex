...
Let us consider an algorithm for OLO over one-dimensional Hilbert
...
Compared to the one dimensional case, the only hard part is to show an analogue of
...
is the content of the Lemma~\ref{lemma:recursion_hilbert}, whose proof is in Appendix~\ref{section:hilbert-space-reduction}. The proof relies
...
We are almost done, the only property we need is that if $F$ is an even function then Fenchel conjugate of
$F(\norm{\cdot})$ is $F^*(\|\cdot\|)$; see \citet[Example 13.7]{BauschkeC2011}.
Hence, we apply Lemma~\ref{lemma:reward-regret} to the function $F(x) = F_T(\norm{x}) -
\epsilon$ and we obtain the regret bound.
