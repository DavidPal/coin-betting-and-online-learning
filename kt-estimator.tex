...
In the previous two sections we shown that a coin-betting potential with a guaranteed rapid growth of the wealth will give good regret guarantees for \ac{OLO} and \ac{LEA}.
In this section we show that the optimal Krichevsky-Trofimov (KT) estimator has associated a
sequence of excellent coin-betting potentials, which we call \emph{KT
potentials}. We then prove corollaries of the regret bounds for ac{OLO} over Hilbert
space and \ac{LEA} that we proved in previous sections, obtaining optimal regret bounds.
...
\qquad \qquad \text{$t \ge 0$, \ $x \in \left(- \, \frac{t+1}{2}, \frac{t+1}{2}\right)$,}
\end{equation}
...
9.7]{Cesa-BianchiL06}. The potential is defined on the interval.
...
\FOR{$t=1,2,\dots$}
\STATE{Predict with $w_t \leftarrow \frac{1}{t} \left(\epsilon + \sum_{i=1}^{t-1} \langle g_t, w_t \rangle \right) \sum_{i=1}^{t-1} g_i$}
%\STATE{Predict $w_t$}
...
requires to know the number of rounds $T$ in advance. The shifted potential is
defined as
$$
