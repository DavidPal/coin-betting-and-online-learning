\section{Krichevsky-Trofimov Estimator and Its Applications}
...
\FOR{$t=1,2,\dots$}
\STATE{Set $w_t \leftarrow \frac{\sum_{i=1}^{t-1} g_i}{t} \left(\epsilon + \sum_{i=1}^{t-1} \langle g_t, w_t \rangle \right)$}
\STATE{Predict $w_t$}
\STATE{Receive vector $g_t \in \H$ such that $\|g_t\| \le 1$}
...
= \frac{\sum_{i=1}^{t-1} g_i}{t} \left(\epsilon + \sum_{i=1}^{t-1} \langle g_t, w_t \rangle \right)\; .
$$
...
defined as
$$
F_t(x) = \frac{2^t \cdot \Gamma(T/2 + 1) \Gamma(\frac{t+T/2+1}{2} + \frac{x}{2}) \Gamma(\frac{t+T/2+1}{2} - \frac{x}{2})}{\Gamma(\frac{T/2+1}{2})^2 \cdot \Gamma(t+T/2+1)} \; .
$$
The reason for its name is that, up to a multiplicative constant, $F_t$ is equal
to the KT potential shifted in time by $T$, i.e., $t$ is replaced by $T/2+t$.
...
$$
\end{corollary}
Changing $T/2$ in Algorithm~\ref{algorithm:kt-experts} to another fraction of $T$ is possible to trade-off the leading constant $3$ for the constant $4$ present in the square root.
