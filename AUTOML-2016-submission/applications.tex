...
only single pass over the dataset and it doesn't have any tuning parameters.
Yet, it has almost the same guarantee \eqref{equation:optimal-rate}
\emph{without knowing $\norm{w^*}$ or optimal regularization parameter
$\lambda$ or learning rate, or any other tuning parameter}.

\begin{algorithm}[t]
\caption{Averaging algorithm based on KT potential \label{algorithm:averaged-olo}}
\begin{algorithmic}[1]
{
...
\STATE{Compute $\ell_i = \frac{\partial f(w, X_i, Y_i)}{\partial w}|_{w=w_{i-1}}$}
