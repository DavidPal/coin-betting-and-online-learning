...
{
\REQUIRE{Algorithm A, prior distribution $\pi \in \Delta_N$}
\FOR{$t=1,2,\dots,T$}
\STATE{For each $i \in [N]$, set $w_{t,i}$ according to copy $i$ of algorithm A}
...
initial endowment $1$.
%\footnote{Any initial endowment $\epsilon > 0$ can be
...
% & \le f_T^{-1}\left(\sum_{i=1}^N u_i \ln \left[ F_T(G_{T,i}) \right]\right) \qquad \text{(by concavity of $f_T$)} \\
