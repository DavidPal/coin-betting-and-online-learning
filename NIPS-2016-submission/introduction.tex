...
\midrule
Hedge~\cite{Freund-Schapire-1997}, $\eta=\tfrac{U}{\sqrt{T}}$ & $\scO(\sqrt{T U})$ for any $u \in \Delta_N$ s.t. $\sqrt{\KL{u}{\pi}} \le U$ & $\scO(N)$ &  \\
...
\cite{Foster-Rakhlin-Sridharan-2015} & $\scO(\sqrt{T \left(1+\KL{u}{\pi}\right)})$, $\forall u \in \Delta_N$ & $\scO(N \max_{i} \ln \ln \frac{1}{\pi_i})$\footnotemark[3] & \checkmark & \checkmark \\
