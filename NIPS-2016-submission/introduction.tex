...
function, is solved through a reduction to 
...
However, to achieve optimal regret, most of the online algorithms still require to set
hyperparameters, usually learning rates. 
For example, to obtain the optimal bound in \ac{OGD}, the learning rate has to be set with the knowledge of the norm of the competitor $u$, first entry in Table~\ref{table:bounds}. In the same way, in Hedge the optimal learning rate depends on the KL divergence bewteen the prior weighting $\pi$ and the unknown competitor $u$, forth entry in Table~\ref{table:bounds}.
...
respectively, without the need to tune parameters.  Even if we now know how to analyze these two
families of algorithms~\cite{Foster-Rakhlin-Sridharan-2015}, their \emph{design} and underlying intuition is still a challenge.
...
\midrule
Hedge~\cite{Freund-Schapire-1997}, $\eta=\sqrt{\tfrac{\ln N - U}{T}}$ & $\scO\left(\sqrt{T (\ln N -U)}\right)$ for any $u \in \Delta_N$ s.t. $H(u) \geq U$ & $\scO(N)$ &  \\
...
\cite{Foster-Rakhlin-Sridharan-2015} & $\scO\left(\sqrt{T \left(1+\KL{u}{\pi}\right)}\right)$, $\forall u \in \Delta_N$ & $\scO(N \max_{u \in \Delta_N} \ln \KL{u}{\pi})$\footnotemark[3] & \checkmark & \checkmark \\
...
All of them are defined in Setion~\ref{sec:prel}, while $\eta$ is the learning rate.}}
\label{table:bounds}
\end{center}
\end{table}
...
\footnotetext[2]{The proof in \cite{Koolen-van-Erven-2015} can be modified to prove a KL divergence bound, as the author himself points out in his blog \url{http://blog.wouterkoolen.info/Squint_PAC/post.html}.}
\footnotetext[3]{A slighly modified version of algorithm in \cite{Foster-Rakhlin-Sridharan-2015} can be implemented with the stated time complexity~\cite{Foster:private}.}
...
algorithms for \ac{OLO} and \ac{LEA} are new, simple, and natural. They also have optimal worst-case guarantees on
