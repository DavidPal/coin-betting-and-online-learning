...
$\{F_t\}_{t=0}^\infty$ initial endowment\footnote{Any initial endowment $\epsilon > 0$ can be
...
algorithm with prior $\pi \in \Delta_N$ that predicts at each round with $p_t$ satisfies
\[
...
\end{theorem}
The proof is in
Appendix~\ref{section:appendix-expert-reduction}.
It is based on the fact that \eqref{eq:gradients_experts_reduction} guarantees that $\sum_{i=1}^N \pi_i \widetilde g_{t,i} w_{t,i} \le 0$ and on a simple modification of the change
