...
\[
\Wealth_t \ge \frac{\max_{\beta \in [-1,1]} \Wealth_t(\beta)}{2\sqrt{t}} \; ,
\]
...
factors~\citep{Cesa-Bianchi-Lugosi-2006} and mirrors the guarantee of
the Kelly bet.
...
assuming the knowledge of the value of $\sum_{t=1}^T g_t$ beforehand and
betting the fixed fraction $(\sum_{t=1}^T g_t)/T$ at each round, gives the
wealth $\epsilon \exp\left(T \KL{\tfrac{1}{2}+\tfrac{\sum_{t=1}^T g_t}{2
