...
It has the peculiar property to satisfy the inequality in point (3) of Definition~\ref{definition:potential}
with equality when $g_t\in \{-1,1\}$, i.e. $F_t(x+1)=(1+\tfrac{x}{t}) \, F_{t-1}(x)$ and $F_t(x-1)=(1-\tfrac{x}{t}) \, F_{t-1}(x)$.
In fact, Theorem~\ref{theorem:kt-potential} stated in
...
\eqref{equation:kt-estimator-potential}. The proof can be found in the Appendix~\ref{section:corollaries_reductions}. The only technical
...
\end{corollary}
%
It is worth noting the simplicity of Algorithm~\ref{algorithm:kt-hilbert-space-olo} and the fact that the regret bound is optimal~\cite{Streeter-McMahan-2012,Orabona-2013}.
...
\]
\end{corollary}
This bound matches the best known guarantees on worst-case regret and per-round time complexity, see Table~\ref{table:bounds}.
