...
\FOR{$t=1,2,\dots,T$}
\STATE{For each $i \in [N]$, set $w_{t,i} \leftarrow \left(1 + \sum_{j=1}^{t-1} \widetilde g_{j,i} w_{j,i} \right) \sum_{j=1}^{t-1} \widetilde g_{j,i}$}
...
To use the construction in Section~\ref{section:reduction-experts}, we need an OLO algorithm for the one-dimensional Hilbert space $\R$.
...
= \frac{\sum_{i=1}^{t-1} \widetilde g_i}{\delta+t} \left(1 + \sum_{j=1}^{t-1} \widetilde g_j w_j \right) \; .
\]
Then, following the construction in Section~\ref{section:reduction-experts}, we
arrive at the final algorithm, Algorithm~\ref{algorithm:kt-experts}. Note that, as observed in Section~\ref{section:reduction-experts}, $p_t$ does not change if we multiply $w_{t,i}$ by $t+\delta$, hence we can remove it from the algorithm, making the algorithm independent of $\delta$.
...
\[
\forall u \in \Delta_N \qquad \qquad \Regret_T(u) \le \min_{\alpha>0} \sqrt{2 (1+\alpha) T \left(\KL{u}{\pi} + 3 + \tfrac{1}{2} \ln \left(1+\tfrac{1}{\alpha}\right)\right)} \; .
\]
\end{corollary}
%
...
We want to stress that $\alpha$ is not a parameter: The regret bounds trade-off automatically between the multiplicative and additive constants.
